...
export async function deploy(wallet: Wallet, chainA: any, chainB: any) {
  ...
  const callContractWithTokenSenderFactory =
    new CallContractWithTokenSenderFactory(walletA);
  const callContractWithTokenReceiverFactory =
    new CallContractWithTokenReceiverFactory(walletB);

  const callContractWithTokenSender =
    await callContractWithTokenSenderFactory.deploy(
      chainA.gateway,
      chainA.gasService
    );
  const callContractWithTokenReceiver =
    await callContractWithTokenReceiverFactory.deploy(
      chainB.gateway,
      chainB.gasService
    );
